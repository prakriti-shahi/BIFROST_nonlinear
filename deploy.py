...
def get_fiber_params(dep):
    """Extract key fiber parameters from a deployment.

    Finds the first FiberLength or SpunFiberLength object and reads
    its computed properties.  Also pulls config-level values.

    Returns
    -------
    dict
        Keys: ``'wavelength_m'``, ``'m_GeO2'``, ``'gamma'`` (W⁻¹m⁻¹),
        ``'n_eff'``, ``'v'``, ``'Aeff'`` (m²), ``'n2'`` (m²/W),
        ``'r0'``, ``'r1'``, ``'epsilon'``.
    """
    cfg = dep['raw_config']
    w0 = cfg['wavelength_nm'] * 1e-9
    m0 = cfg['fiber']['core_GeO2_mol_fraction']

    # Find first real fiber segment
    ref = None
    for f in dep['fibers']:
        if hasattr(f, 'gamma'):
            ref = f
    ...
    if ref is None:
        raise RuntimeError("No FiberLength or SpunFiberLength found")

    return {
        'wavelength_m': w0,
        'm_GeO2': m0,
        'gamma': ref.gamma,
        'n_eff': ref.beta / (2 * np.pi / ref.w0),
        'v': ref.v,
        'Aeff': ref.Aeff,
        'r0': ref.r0,
        'r1': ref.r1,
